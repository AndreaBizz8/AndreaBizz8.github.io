...
/*ELEMENT FROM LYRICIST TABLE*/
$queryPlaywrightInfo = 'SELECT * FROM show_playwright_info ORDER BY show_playwright_name';
$statement1 = $conn->prepare($queryPlaywrightInfo);
$statement1->execute();
$playwrightList = $statement1->fetchAll();
$statement->closeCursor();
?>
<html>
	<head>
		...
		<div class="container">
			<div class="row">
				<div class="col">
					...
					<div class="container">
						<form action="connectPlaywright.php" method="post" id="connectPlaywright">
							<p class="h2 paragraphTitle">Data Already In Database</p>
							<p class="h2 paragraphTitle">Show</p>
							<div class="row">
								<select name="show_ID">
									<?php foreach ($showlist as $listOfShow): ?>
										<option value="<?php echo $listOfShow['show_ID']; ?>"><?php echo $listOfShow['show_title']; ?></option>
									<?php endforeach ; ?>
								</select>
							</div>
							<p class="h2 paragraphTitle">Playwright</p>
							<div class="row">
								<select name="playwright_ID">
									<?php foreach ($playwrightList as $listOfPlaywrighter): ?>
										<option value="<?php echo $listOfPlaywrighter['show_playwright_ID']; ?>"><?php echo $listOfPlaywrighter['show_playwright_name']; ?></option>
									<?php endforeach ; ?>
								</select>
							</div>
				...
				<div class="col">
					<div class="container">
						<form action="addPlaywrightToDatabase.php" method="post" id="addPlaywright">
							<p class="h2 paragraphTitle">Add Playwright if not present</p>
							<p class="h2 paragraphTitle">Playwright</p>
							<div class="row">
								<label>Composer Name</label>
								<input type="text" name="show_playwright_name" id="show_playwright_name">
							</div>
							<input type="submit" value="register">
						</form>
					</div>
			...
			<div class="row">
				<div class="col"><a href="addcomposer.php">ADD COMPOSER</a></div>
				<div class="col"><a href="addplaywright.php">ADD PLAYWRIGHT</a></div>
				<div class="col"><a href="addlyrics.php">ADD another lyrics</a></div>
			</div>
		</div>
