...
$queryLyricsInfo = 'SELECT * FROM show_lyrics_info ORDER BY show_lyrics_name';
$statement1 = $conn->prepare($queryLyricsInfo);
$statement1->execute();
$lyricisList = $statement1->fetchAll();
$statement->closeCursor();
?>
<html>
	<head>
	...
	<main class="mt-5 pt-5" role="main">
		<div class="container">
			<div class="row">
				<div class="col">
					<div class="container">
						<form action="connectLyrics.php" method="post" id="connectLyrics">
							<p class="h2 paragraphTitle">Data Already In Database</p>
							<p class="h2 paragraphTitle">Show</p>
							<div class="row">
								<select name="show_ID">
									<?php foreach ($showlist as $listOfShow): ?>
										<option value="<?php echo $listOfShow['show_ID']; ?>" name="show_ID" id="show_ID"><?php echo $listOfShow['show_title']; ?></option>
									<?php endforeach ; ?>
								</select>
							</div>
							<p class="h2 paragraphTitle">Lyricist</p>
							<div class="row">
								<select name="lyrics_ID">
									<?php foreach ($lyricisList as $listOfLyrics): ?>
										<option value="<?php echo $listOfLyrics['show_lyrics_ID']; ?>" name="lyrics_ID" id="lyrics_ID"><?php echo $listOfLyrics['show_lyrics_name']; ?></option>
									<?php endforeach ; ?>
								</select>
							</div>
				...
				<div class="col">
					<div class="container">
						<form action="addLyricistInfoToDatabase.php" method="post" id="addLyrics">
							<p class="h2 paragraphTitle">Add Lyrics if not present</p>
							<p class="h2 paragraphTitle">Lyricist</p>
							<div class="row">
								<label>Lyricist Name</label>
								<input type="text" name="show_lyrics_name" id="show_lyrics_name">
							</div>
